...
void construire_message(char *message, char motif, int lg, int num_msg)
{
	...
	// Ajout du motif de dÃ©part de message
...
{
	// Inititialisation des variables utilisÃ©s du cotÃ© UDP / TCP /
	int c;
	extern char *optarg;
	extern int optind;
	int nb_message = -1; // Nb de messages Ã  envoyer ou Ã  recevoir, par dÃ©faut : 10 en Ã©mission, infini en rÃ©ception
	int source = -1;	 // 0 = puits, 1=source
	int l = 30;			 // longueur max Ã©mis ou reÃ§u de base Ã  30 octets
	int u = 0;			 // 0 = TCP, 1=UDP
	int sock, sock_bis;
	...
	int lus = 0; // nbr message lu par TCP
	// Variable par dÃ©fault si pas dÃ©fini dans les paramÃ¨tres
	char *nom = "localhost";
	int port = 9000;

	// RÃ©cupÃ©ration des options rentrÃ©es en entrÃ©es de la fonction

	while ((c = getopt(argc, argv, "upn:sl:")) != -1)
	{
		switch (c)
		{
		case 'p':
	...
	char *pmesg = malloc(sizeof(char) * lg_M); // Allocation dynamique pour s'adapter Ã  l'option -l

	// RÃ©cupÃ©ration du nom de domaine et du numÃ©ro de port
	...
	// Emplacement initial des fonctions permettant de vÃ©rifier si on est en source / puit etc
	if (source == -1)
	{
		printf("usage: cmd [-p|-s][-n ##]\n");
		exit(1);
	...
	//-------------SOURCE SOUS UDP------------------------------------------
	if (u == 1 && source == 1)
	{ // crÃ©ation socket pour source
		if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		{
			printf("echec crÃ©ation socket dans source \n");
			exit(1);
		}
		...
		// RÃ©cupÃ©ration de l'IP correspondant au nom de domaine
	...
	{ // crÃ©ation socket pour source
		...
		{
			printf("echec crÃ©ation socket dans source TCP\n");
			exit(1);
		}
		...
		{
			printf("ProblÃ¨me fin de connexion dans la source\n");
		}
		close(sock);
	}
	...
	if (source == 0 && u == 1)
	{
		...
		// crÃ©ation socket pour puits
		if ((sock = socket(AF_INET, SOCK_DGRAM, 0)) == -1)
		{
			printf("echec crÃ©ation socket dans puits \n");
			exit(1);
		}
		...
		{
			printf("Ã©chec du bind\n");
			exit(1);
		}

		// mis en Ã©coute
		// listen(sock,20); Pas nÃ©cessaire, seulement pour TCP
		// faisait partie de la V1 et pas nÃ©cessaire
		...
		while (nb_message == -1 || nb_m <= nb_message)
		{
			...
			if (lu > 0)
			{
				printf("PUITS: Reception nÂ°%d (%d) [%.*s]\n", nb_m, lu, lu, pmesg);
				nb_m++;
			}
		}
	}
	...
	if (source == 0 && u == 0)
	{
		...
		// Mise en place de la crÃ©ation de nouveaux processus si nÃ©cÃ©ssaire
		while (1)
		{ // acceptation de la connexion
			if ((sock_bis = accept(sock, (struct sockaddr *)&adr_distant, &lg_adr_distant)) == -1)
			{
				printf("Ã©chec du accept\n");
				continue; // On passe au client suivant en cas d'erreur
			}

			// CrÃ©ation d'un processus fils pour gÃ©rer ce client spÃ©cifiquement
			int pid;
			...
			case -1:
				// ERREUR
				printf("Ã©chec du fork\n");
				close(sock_bis);
				break;

			case 0:
				// PROCESSUS FILS : S'occupe de la lecture
				close(sock); // Le fils n'a pas besoin du sock d'Ã©coute
				...
				// Fermeture propre : on coupe la lecture, puis on dÃ©truit le socket
				shutdown(sock_bis, 0);
				close(sock_bis); // Le fils ferme son socket de dialogue une fois terminÃ©
				exit(0);

			default:
				// PROCESSUS PERE : Continue d'Ã©couter
				close(sock_bis); // Le pÃ¨re n'a pas besoin du sock de dialogue
				break;
			}
		}
	}
